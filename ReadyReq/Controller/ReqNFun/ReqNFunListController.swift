...
class ReqNFunListController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, GenericProtocol {
    ...
    let urlPath: String = MyUserDefaults.readUDHTTP() + "://" + MyUserDefaults.readUDServerIp() + ":" + String(MyUserDefaults.readUDPortHTTP()) + "/readyreq/reqnfun_frag_list.php"
    let generic = Generic()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let destinationVC = navController.topViewController as! ReqNFunTabBarController
        if(segue.identifier == "showEditReqNFun"){
            getIdSelected(idSelected: &idSelected)
            destinationVC.idReqNFun = idSelected
        }
        if(segue.identifier == "showNewReqNFun"){
            destinationVC.idReqNFun = idSelected
        }
    }
