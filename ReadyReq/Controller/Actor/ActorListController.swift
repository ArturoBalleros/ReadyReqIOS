...
class ActorListController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, GenericProtocol {
    ...
    let urlPath: String = MyUserDefaults.readUDHTTP() + "://" + MyUserDefaults.readUDServerIp() + ":" + String(MyUserDefaults.readUDPortHTTP()) + "/readyreq/actor_frag_list.php"
    let generic = Generic()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let destinationVC = navController.topViewController as! ActorTabBarController
        if(segue.identifier == "showEditActor"){
            getIdSelected(idSelected: &idSelected)
            destinationVC.idActor = idSelected
        }
        if(segue.identifier == "showNewActor"){
            destinationVC.idActor = idSelected
        }
    }
