...
class PackageListController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, GenericProtocol {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let destinationVC = navController.topViewController as! DataPackageController
        if(segue.identifier == "showEditPackage" ){
            getIdSelected(idSelected: &idSelected)
            destinationVC.idPackage = idSelected
        }
        if(segue.identifier == "showNewPackage" ){
            destinationVC.idPackage = idSelected
        }
    }
