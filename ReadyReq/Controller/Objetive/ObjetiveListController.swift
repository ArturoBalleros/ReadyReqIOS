...
class ObjetiveListController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, GenericProtocol {
    ...
    let urlPath: String = "http://" + MyUserDefaults.readUDServerIp() + ":8080/readyreq/objet_frag_list.php"
    let generic = Generic()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let destinationVC = navController.topViewController as! ObjetiveTabBarController
        if(segue.identifier == "showEditObjet"){
            getIdSelected(idSelected: &idSelected)
            destinationVC.idObjetive = idSelected
        }
        if(segue.identifier == "showNewObjet"){
            destinationVC.idObjetive = idSelected
        }
    }
