...
class ReqFunListController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, GenericProtocol {
    ...
    let urlPath: String = MyUserDefaults.readUDHTTP() + "://" + MyUserDefaults.readUDServerIp() + ":" + String(MyUserDefaults.readUDPortHTTP()) + "/readyreq/reqfun_frag_list.php"
    let generic = Generic()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let destinationVC = navController.topViewController as! ReqFunTabBarController
        if(segue.identifier == "showEditReqFun"){
            getIdSelected(idSelected: &idSelected)
            destinationVC.idReqFun = idSelected
        }
        if(segue.identifier == "showNewReqFun"){
            destinationVC.idReqFun = idSelected
        }
    }
