...
//  ReadyReq
//
//  Created by Arturo Balleros Albillo on 26/11/2018.
...
class GroupListController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, GenericProtocol {
    ...
    let urlPath: String = MyUserDefaults.readUDHTTP() + "://" + MyUserDefaults.readUDServerIp() + ":" + String(MyUserDefaults.readUDPortHTTP()) + "/readyreq/group_frag_list.php"
    let generic = Generic()
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        let destinationVC = navController.topViewController as! DataGroupController
        if(segue.identifier == "showEditWorker" ){
            getIdSelected(idSelected: &idSelected)
            destinationVC.idWorker = idSelected
        }
        if(segue.identifier == "showNewWorker" ){
            destinationVC.idWorker = idSelected
        }
    }
